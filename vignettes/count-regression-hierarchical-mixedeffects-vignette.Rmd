---
title: " Heirarchical / Mixed Effects"
...
fixef(lme_fit1)
plot(ranef(lme_fit1),main="fitted random effects")
coef.re <- coef(lme_fit1)
plot(coef.re[[1]][,1],coef.re[[1]][,2],main="Coefficients ",ylab="(Intercept)",xlab="x")
...
## Bayesian hierarchical analysis using JAGS
...
plot(ranef(lme_fit2)[[1]][,1],ranef(lme_fit2)[[1]][,2],main="fitted random effects")
...
### Bayesian analysis using JAGS of a random-coefficients model without correlation between intercept and slope
...
This is very helpful for more complex models b/c it's easy to make mistakes.  A good way to check the JAGS analysis for a custom model that is needed for a particular phenomenon is to simulate the data and run the JAGS model on that data
...
## Finally, we simulate data from a model with 3 correlated ranefs and then provide the R and JAGS analyses of this data
...
# TBD compare the prior and posterior distributions for the (derived) ranef distribution parameters:
```
